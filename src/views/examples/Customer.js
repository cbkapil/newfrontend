...
const Company = () => {
  ...
  const handleRowUpdate = async (newData, oldData, resolve) => {
    ...
    let result = await fetch(`/updatecustomer/${newData._id}`, {
      method: "put",
      body: JSON.stringify({
        newData,
      }),
    ...
    setcustomerdata([...dataUpdate]);
    resolve();
  };

  const handleRowDelete = async (oldData, resolve) => {
    console.log("ohohohooo", oldData._id);
    let result = await fetch(`/deletee/${oldData._id}`, {
      method: "delete",
    ...
    result = await result.json();
    console.log(result);
    const dataDelete = [...customerdata];
    const index = oldData.tableData.id;
    dataDelete.splice(index, 1);
    setcustomerdata([...dataDelete]);
    resolve();
  };
  ...
  const getFirm = async () => {
    let result = await fetch("/getfirm", {
      method: "GET",
  ...
  const getcustomerdata = async () => {
    let result = await fetch("/getcustomerdata", {
      method: "GET",
  ...
  const success = () => {toast.success('Firm Data Added Successfully !!!!', {
    ...
    const firmSubmit = async (e) => {
      ...
      let result = await fetch("/createparty", {
        method: "post",
        body: JSON.stringify({
          customername,
          email,
