...
//**************************Get Customer List****************************** */
  const getParty = async () => {
    let result = await fetch("/getparty", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + token,
...
//**************************************************** */
  const formSubmit = async (e) => {
    ...
    let result = await fetch("/createchequecollection", {
      method: "post",
      body: JSON.stringify({
        customerid,
        chequeFirmid,
...
//******************Function to Get Customer Bank********************************* */
  const getCustomerBank = async (index) => {
    console.log(index)
    if (!customerid) return;
    else {
      console.log(customerid);
      let result = await fetch("/getcustomerbank", {
        method: "post",
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + token,
      ...
      if(editable===false){
         var newFormValues = [...formValues];
          newFormValues[indexx].customerbank = result.customerbank[0].partybankname
          newFormValues[indexx].customerbranch = result.customerbank[0].partybranch
        
          console.log(result.customerbank[0].partybankname);
      }
...
//*********************************************************** */
    const cusomerbankadded = async (e) => {
      ...
      let result = await fetch("/createpartybank", {
        method: "post",
        body: JSON.stringify({
          formValues,
        }),
...
//********************************************************** */
  ...
  useEffect(() => {
   getFirmChequeSlip();
    ...
    getData();
    getCustomerBank()
  }, [customerid,customername,firstbank,firstbranch,editable,indexx])
                  ...
                  <div className="form-box bank-form d-flex flex-column p-3">
                    ...
                    <div className="input">
                        <label
                          ...
                          name="customerbank"
                          value={formValues[index].customerbank}
                          onChange = {(e)=> {handleChangee(index, e);
                            seteditable(true)
                          
                          formValues[indexx].customerbank=e.target.value
                        ...
                        <label
                          ...
                          <Input
                           autocomplete ="off"
                              ...
                              type="text"
                              value={  formValues[index].customerbranch}
                              onChange={(e) => {  
                               
                                handleChangee(index, e);
                                seteditable(true)
                                
                                formValues[indexx].customerbranch=e.target.value
